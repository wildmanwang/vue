<template>
    <div class="app-container">
        ...
        <el-col :span="8">
            <el-input v-model="qName" placeholder="请输入供应商名称" size="small" @change="fetchData"></el-input>
        </el-col>
        ...
        <pagination v-show="total>0" :total="total" :page.sync="listPage.page" :limit.sync="listPage.limit" @pagination="fetchData" />
        ...
        <el-dialog :title="dataDialogTitle" :visible.sync="dataDialogVisible">
            <el-form ref="dataForm" :rules="rules" :model="temp" label-position="left" label-width="70px"
                ...
                <el-form-item label="简称" prop="simple_name">
                    <el-input v-model="temp.simple_name" :disabled="dataDialogStatus === '查看' ? true : false" />
                </el-form-item>
                <el-form-item label="代码" prop="code">
                    <el-input v-model="temp.code" :disabled="dataDialogStatus === '查看' ? true : false" />
                </el-form-item>
...
export default {
    ...
    methods: {
        fetchData() {
            ...
            fetchBasicDataList({ "dataType": "supplier", "query": this.listQuery, "page": this.listPage }).then(response => {
        ...
        newDataSave() {
            ...
            this.$refs['dataForm'].validate((valid) => {
                if (valid) {
                    this.listLoading = true
                    newBasicData({ "dataType": "supplier", "data": this.temp }).then(response => {
        ...
        modifyDataSave() {
            ...
            this.$refs['dataForm'].validate((valid) => {
                if (valid) {
                    this.listLoading = true
                    modifyBasicData({ "dataType": "supplier", "data": this.temp }).then(response => {
                        if (response.data.result) {
                            if (this.temp['status'] === 1) {
                                this.temp['enum_status'] = '正常'
                            } else {
        ...
        deleteDataSave(refname) {
            // 删除保存
            this.listLoading = true
            deleteBasicData({ "dataType": "supplier", "id": this.row.id }).then(response => {
                this.$refs[refname].doClose()
                if (response.data.result) {
                    this.$notify({
                        title: 'Success',
